...
execute_ilo_command() {
    ...
    while [[ $retry -lt $COMMAND_RETRIES ]]; do
        ...
        if timeout "$CONNECTION_TIMEOUT" "${SSH_EXEC[@]}" "$cmd" 2>&1; then
            log_message "DEBUG" "Command successful: $cmd"
            return 0
        else
            ((retry++))
            log_message "WARN" "Command failed (attempt $retry/$COMMAND_RETRIES): $cmd"
